...
func (r *AuthDB) isUsernameTaken(username string) (bool, error) {
	...
	query := fmt.Sprintf("SELECT EXISTS(SELECT 1 FROM %s WHERE LOWER(username) = LOWER(?))", usersTable)
...
func (r *AuthDB) CreateUser(user todo.User) error {
	...
	query := fmt.Sprintf("INSERT INTO %s (username, password_hash) VALUES (?, ?)", usersTable)
...
func (r *AuthDB) NewRefreshToken(token, userId string, expiresAt time.Time) error {
	...
	if err == sql.ErrNoRows {
		...
		query = fmt.Sprintf("INSERT INTO %s (refresh_token, expires_at) VALUES (?, ?) RETURNING id", tokensTable)
		...
		query = fmt.Sprintf("INSERT INTO %s (user_id, token_id) VALUES (?, ?)", usersTokensTable)
		...
		query = fmt.Sprintf("UPDATE %s SET refresh_token = ?, expires_at = ? WHERE id = ?", tokensTable)
...
func (r *AuthDB) CheckRefreshToken(userId, refreshToken string) error {
	var tokenId string
	query := fmt.Sprintf("SELECT ut.token_id FROM %s ut WHERE ut.user_id = ?", usersTokensTable)
	...
	query = fmt.Sprintf("SELECT t.refresh_token, t.expires_at FROM %s t WHERE t.id = ?", tokensTable)
